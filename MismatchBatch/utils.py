...
def get_gcn(in_size, out_size, hid: int=64, device='cpu'):
...
def eval_adj(model, adj, feat, label, test_mask):
    model.eval()
    pred = model(feat, adj)
...
def get_samples(A, X, T, train_mask, num_samples, sample_size, device='cpu'):
...
def get_dataloader(adj, feat, label, train_mask, num_samples, sample_size, batch_size):
    subgraph_feats, subgraph_adjs, subgraph_labels, subgraph_train_masks = get_samples(adj, feat, label, train_mask, num_samples, sample_size)
    return DataLoader(
    TensorDataset(subgraph_feats, subgraph_adjs, subgraph_labels, subgraph_train_masks), 
    batch_size=batch_size, shuffle=True)


def load_data(dataset, seed, root='./tmp/', device='cpu'):
    ...
    return adj.to(device), feat.to(device), labels.to(device), train_mask, val_mask, test_mask
