...
parser.add_argument('--attack_method', type=str, default='heuristic', choices=[
...
config = yaml.load(open('config3.yml'), Loader=SafeLoader)[args.dataset]
...
top = torch.from_numpy(entropies).topk(k=num_tasks - 1).indices

tasks[0] = labels
for i, top_entropy in enumerate(top):
  tasks[i + 1] = discretize(feat[:, top_entropy].contiguous(), n_bins=3).to(device)
...
accs = torch.zeros((tasks.shape[0], 2))
...
for i, acc in enumerate(accs):
  results[f'd_g0_{i}'] = acc[0].item()
  results[f'd_gX_{i}'] = acc[1].item()

export.saveData('./multitask4.csv', results)
