...
class Grace(nn.Module):
    ...
    def forward(self, graph1, graph2, feat1, feat2, batch_size=0, device=None, useWeight=False):
        # encoding
        if useWeight:
            h1 = self.encoder(graph1, feat1.float(), edge_weight=graph1.edata['weight'])
            h2 = self.encoder(graph2, feat2.float(), edge_weight=graph2.edata['weight'])
        else:
            h1 = self.encoder(graph1, feat1.float())
            h2 = self.encoder(graph2, feat2.float())
